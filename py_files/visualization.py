...
player_data["weight_kg"] = (
    player_data["weight"].astype(float) * 0.453592
)  # Перетворення ваги з фунтів у кілограми

# new players feature
players["age"] = pd.to_datetime(players["born"]).apply(
    lambda x: (pd.to_datetime("today").year - x.year)
)  # Розрахунок віку гравців

# new season_stats feature
season_stats["PTS_per_game"] = (
    season_stats["PTS"] / season_stats["G"]
)  # Розрахунок середньої кількості очок на гру
